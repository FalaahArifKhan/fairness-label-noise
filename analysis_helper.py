...
def compute_metric(yhat, y_test, test_groups, metric):
    res = pd.DataFrame({})
    for group in test_groups.keys():
        if metric == 'Equal_Opportunity':
            temp = TPR_diff(yhat, y_test, test_groups[group]['values'], test_groups[group]['advantaged'], test_groups[group]['disadvantaged'])
        
        if metric == 'Disparate_Impact':
            temp = DisparateImpact(yhat, y_test, test_groups[group]['values'], test_groups[group]['advantaged'], test_groups[group]['disadvantaged'])
            
        if metric == 'Base_Rate':
            temp = BaseRates(yhat, y_test, test_groups[group]['values'], test_groups[group]['advantaged'], test_groups[group]['disadvantaged'])

        if metric == 'Accuracy':
            temp = Accuracy_diff(yhat, y_test, test_groups[group]['values'], test_groups[group]['advantaged'], test_groups[group]['disadvantaged'])
